...
public class ClassesIMPL implements ClassesDAO {
    ...
    @Override
    public Classes getBySingleId(String id) {
        String sql = "SELECT * FROM lophoc WHERE id = ?";
        return jdbcTemplate.query(sql, new ResultSetExtractor<Classes>() {
            @Override
            public Classes extractData(ResultSet rs) throws SQLException, DataAccessException {
                Classes clas = new Classes();
                while (rs.next()) {
                    clas.setId(rs.getString("id"));
